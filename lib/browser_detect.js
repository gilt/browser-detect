(function (exports) {
  ...
  function extractBrowser (userAgent) {
    ...
    if (/(ms|\b)?(ie|rv)/i.test(userAgent)) {
      name = 'ie';
      try {
      ...
      name = 'chrome';
      try {
        version = parseFloat(userAgent.match(/(?:chrome|crios)\/(\d+\.\d+)/i)[1]);
      } catch (e) {}
  ...
  function analyze () {
    ...
    if (analysis.browser.name !== features.browserName) {
      // TODO
    } else if (analysis.browser.name === 'is' &&
