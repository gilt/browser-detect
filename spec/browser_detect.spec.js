/*global __browserDetect: false, jasmine: false*/
xdescribe('Browser Detect', function () {
  var expected = [
    {
    ...
    {
      userAgent: 'Mozilla/5.0 (compatible; MSIE 9.0; Windows NT 6.1; WOW64; Trident/5.0; chromeframe/12.0.742.112)',
      os: {
        name: 'windows',
        version: 6.1
      },
    ...
    },
    {
      ...
      browser: {
        name: 'safari',
        version: 7.1 // TODO: not sure what this should actually be? probably undefined, but might be 534.11
      },
      chromeframe: false,
      mobile: true
    }
  ...
  describe('extractOS', function () {
    expected.forEach(function (entry) {
      ...
      it('should extract the version "' + entry.os.version + '" from "' + entry.userAgent + '"', function () {
        var v = __browserDetect.extractOS(entry.userAgent).version;
        if (isNaN(entry.os.version)) {
          expect(v).toBeNaN();
        } else {
          expect(v).toBe(entry.os.version);
        }
      });
    });
  });
  ...
  describe('extractBrowser', function () {
    expected.forEach(function (entry) {
      ...
      it('should extract the version "' + entry.browser.version + '" from "' + entry.userAgent + '"', function () {
        var v = __browserDetect.extractBrowser(entry.userAgent).version;
        if (isNaN(entry.browser.version)) {
          expect(v).toBeNaN();
        } else {
          expect(v).toBe(entry.browser.version);
        }
      });
    });
  });

  describe('analyzeUserAgent', function () {
    expected.forEach(function (entry) {
      it('should return an os object (details tested above)', function () {
        expect(__browserDetect.analyzeUserAgent(entry.userAgent).os).toEqual(jasmine.any(Object)); // can't test directly, because toEqual chokes on NaN
      });

      it('should return a browser object (details tested above)', function () {
        expect(__browserDetect.analyzeUserAgent(entry.userAgent).browser).toEqual(jasmine.any(Object)); // can't test directly, because toEqual chokes on NaN
      });

      it('should return a boolean flag for chromeframe', function () {
        expect(__browserDetect.analyzeUserAgent(entry.userAgent).chromeframe).toBe(entry.chromeframe);
      });

      it('should return a boolean flag for mobile', function () {
        expect(__browserDetect.analyzeUserAgent(entry.userAgent).mobile).toBe(entry.mobile);
      });
    });
  });
});
